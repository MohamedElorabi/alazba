                                ...
                                <tbody>
                                    @foreach ($payment_methods as $payment_method)
                                        <tr>
                                            ...
                                            <td>{{ $payment_method->available }}</td>
                                            <td>
                                                <div class="d-flex align-items-center gap-3">
                                                    <a href="{{ route('show.payment_method', $payment_method->id) }}"
                                                        class="btn btn-success"><i class="fa fa-eye"></i> Show</a>

                                                    <a href="{{ route('edit.payment_method', $payment_method->id) }}"
                                                        class="btn btn-primary"><i class="fa fa-edit"></i> Edit</a>
                                                    <form method="post"
                                                        action="{{ route('delete.payment_method', $payment_method->id) }}">
                                                        @csrf
